  ...
  return (
    <section className="py-12 md:py-24 bg-background">
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        <h2 className="text-3xl md:text-5xl font-display font-bold text-primary mb-10 md:mb-16 text-center">Financial Tools</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          <motion.div whileHover={{ y: -5 }} className="bg-[#F8F6F2] p-6 md:p-8 rounded-[28px] border border-white shadow-lg hover:shadow-2xl transition-all">
            <h3 className="text-xl md:text-2xl font-bold mb-4 md:mb-6">EMI Calculator</h3>
            <label className="block mb-3 md:mb-4 text-sm md:text-base">Property Cost: ₹ {price.toLocaleString()}</label>
            ...
            <div className="flex flex-col sm:flex-row gap-6 md:gap-8 items-center">
              <div className="flex-1 text-center sm:text-left">
                <div className="text-xs md:text-sm text-gray-500">Monthly EMI</div>
                <div className="text-2xl md:text-3xl font-bold text-primary">₹ {Math.round(emi).toLocaleString()}</div>
              </div>
