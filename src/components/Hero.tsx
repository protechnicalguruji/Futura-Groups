...
export default function Hero({ onNavigate }: { onNavigate: (page: string, scrollToId?: string) => void }) {
  return (
    <section id="hero" className="relative min-h-[80vh] lg:h-[750px] w-full flex items-center py-12 md:py-20 lg:py-16 px-8 overflow-hidden bg-primary text-white">
      ...
      <div className="relative z-20 flex flex-col justify-center w-full max-w-7xl mx-auto py-8 md:py-0">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 md:gap-16 lg:gap-12">
          <div className="w-full md:w-1/2 flex flex-col justify-center text-center md:text-left items-center md:items-start">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="font-display text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-bold leading-[1.1] mb-4 md:mb-6">
              Trusted Real Estate <span className="text-accent">Solutions</span> for Your Future.
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="text-base md:text-xl lg:text-lg text-gray-200 mb-6 md:mb-8 max-w-lg">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="flex flex-wrap justify-center md:justify-start gap-3 md:gap-4">
              ...
              <button onClick={() => onNavigate('contact', 'contact-form')} aria-label="Book a consultation" className="px-5 md:px-8 py-2.5 md:py-4 bg-white/10 text-white border border-white/30 rounded-[18px] font-sans font-bold hover:bg-white/20 transition-all text-xs md:text-base">Book Consultation</button>
            </motion.div>
          </div>
