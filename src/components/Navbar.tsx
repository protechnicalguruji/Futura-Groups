...
export default function Navbar({ onNavigate }: { onNavigate: (page: string, scrollToId?: string) => void }) {
  ...
  return (
    <motion.nav
      ...
      className="fixed top-3 md:top-5 left-1/2 -translate-x-1/2 w-[92%] h-[60px] md:h-[76px] lg:h-[60px] z-50 flex items-center justify-between px-6 md:px-8 lg:px-5 bg-white/60 backdrop-blur-md border border-white/40 rounded-full shadow-lg"
    >
      <div className="font-display text-xl md:text-2xl lg:text-lg font-bold text-primary cursor-pointer" onClick={() => onNavigate('home')}>FUTURA GROUPS</div>
      <div className="hidden md:flex gap-4 lg:gap-4 font-sans font-medium text-primary lg:text-xs">
      ...
      <div className="hidden md:flex items-center gap-4 lg:gap-2">
        <a href="tel:+918884544588" aria-label="Call Us" className="flex items-center gap-2 px-6 py-3 lg:px-4 lg:py-2 bg-primary text-secondary rounded-[18px] font-sans font-medium hover:bg-secondary hover:text-white transition-all hover:scale-105 shadow-md lg:text-xs">
          <Phone size={18} className="lg:w-3.5 lg:h-3.5" /> Call
        </a>
        <a href="https://wa.me/918884544588?text=Hello Futura Groups, I am interested in your real estate services. Please contact me." target="_blank" aria-label="Book a consultation" className="flex items-center gap-2 px-6 py-3 lg:px-4 lg:py-2 bg-accent text-primary rounded-[18px] font-sans font-medium hover:bg-opacity-90 transition-all hover:scale-105 shadow-md lg:text-xs">
          <CalendarDays size={18} className="lg:w-3.5 lg:h-3.5" /> Book
        </a>
      </div>
